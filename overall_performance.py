...
for repet in range(5):
    ...
    ran = np.linspace(start = 0, stop = len(X_data_all)-1, num = 8000, dtype = int)
    ...
    for k_target in kernel_target_nums:
        ...
        gpr = DGram_optimize(gpr, k_target, step_num)
        # gpr = ConEn_optimize(gpr, k_target, step_num, step_num)

        # gpr = UCB_optimize(gpr, k_target, step_num, step_num)
        # gpr = Error_optimize(gpr, k_target, step_num, step_num)
    ...
    scio.savemat('DGram_learning_results'+ str(repet) +'.mat', { 'kernel_target_nums':kernel_target_nums,\
                                            'MUS':MUS,
                                            'MMS':MMS})
